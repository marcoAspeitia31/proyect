<template> 
  <div class="welcome-container">
    ...
    <div class="branch-info" @click="openModal">
      ...
      <h4 class="branch-address">{{ selectedStore.address }}</h4>
      <h4 class="branch-phone">{{ selectedStore.phone }}</h4>
    </div>
    ...
    <div v-if="isModalOpen" class="modal-overlay" @click.self="closeModal">
      <div class="modal-content">
        ...
        <ul class="store-list">
          <li v-for="store in stores" :key="store.id" class="store-item" @click="selectStore(store)">
            <h3 class="store-division">{{ store.division }}</h3>
            <h3 class="store-name">{{ store.name }}</h3>
            <h4 class="store-address">{{ store.address }}</h4>
            <h4 class="store-phone">{{ store.phone }}</h4>
          </li>
        </ul>
      </div>
    </div>
    ...
    <div v-if="isCustomerModalOpen" class="modal-overlay" @click.self="closeCustomerModal">
      <div class="modal-content">
        ...
        <ul class="customer-list">
          <li v-for="customer in customers" :key="customer.phone" class="customer-item" @click="selectCustomer(customer)">
            <h3 class="customer-name">{{ customer.name }}</h3>
            <h4 class="customer-address">{{ customer.address }}</h4>
            <h4 class="customer-phone">{{ customer.phone }}</h4>
          </li>
        </ul>
      </div>
    </div>
